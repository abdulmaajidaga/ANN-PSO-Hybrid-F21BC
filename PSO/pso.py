...
class ParticleSwarm(object):
    def  __init__(self, num_particles, num_informants,  objective_function,  particle_length, discrete_params = None,
        ...
        self._scale_discrete_variables_limits()
        
        # Reference -> Particle swarm optimization: Velocity initialization
        ...
        self.velocity_array = np.random.uniform(low=-0.5, high=0.5, size=(num_particles, self.particle_length))
    ...
    def _scale_discrete_variables_limits(self):
        ...
        num_discrete_options = self.discrete_params[1]      # options per discrete variables
        ...
        upper_limit = np.ones(num_discrete_variables*num_discrete_options)
        lower_limit = np.zeros(num_discrete_variables*num_discrete_options)
        self.p_max[:, num_continuous_variables:] = upper_limit
        self.p_min[:, num_continuous_variables:] = lower_limit
    ...
    def _normalize_discrete_variables(self):
        ...
        #print(f"Before Normalization: {discrete_section[0][0]}")
        ...
        #print(f"After Normalization: {discrete_section[0][0]}")
